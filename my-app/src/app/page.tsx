...
const NoteCard = React.memo(({ note, handleOpenNote, copyNote, folderId, setDeleteRequest, setHoveredDeleteId, hoveredDeleteId }: {
  note: any,
  handleOpenNote: (id: string) => void,
  copyNote: any,
  folderId: string | null,
  setDeleteRequest: any,
  setHoveredDeleteId: any,
  ...
  return (
    <motion.div
      ...
      className="flex flex-col items-center group opacity-0"
    >
      <div
        onClick={() => handleOpenNote(note.id)}
        className="relative w-full aspect-3/4 mb-4 perspective-[1000px] cursor-pointer"
      >
        {/* Atmosphere & Interaction Glow */}
        <div className={cn(
          "absolute inset-0 bg-cyan-500/10 blur-3xl rounded-full opacity-40 group-hover:opacity-80 transition-all duration-700",
          hoveredDeleteId === note.id && "bg-red-500/20"
        )} />

        {/* Back Panel (Static Glass) */}
        <div className={cn(
          "absolute inset-0 bg-[#050505]/40 backdrop-blur-md rounded-[32px] border border-cyan-500/10 shadow-[0_0_30px_rgba(34,211,238,0.15)] group-hover:shadow-[0_0_50px_rgba(34,211,238,0.3)] transition-all duration-500",
          hoveredDeleteId === note.id && "border-red-500/30 shadow-[0_0_40px_rgba(239,68,68,0.2)]"
        )} />

        {/* Glowing Document Edge (Revealed on hover) */}
        <motion.div
          className="absolute inset-x-10 top-6 h-full bg-white/10 backdrop-blur-sm rounded-t-xl border-x border-t border-white/30 shadow-[0_0_15px_rgba(255,255,255,0.2)] z-0"
        ...
        <motion.div
          whileHover={{ rotateX: -25, y: -10 }}
          transition={{ type: "spring", stiffness: 300, damping: 20 }}
          className={cn(
            "absolute inset-0 bg-[#02040a] backdrop-blur-xl border border-cyan-500/30 rounded-[32px] z-10 origin-bottom flex items-center justify-center transition-all duration-500 overflow-hidden",
            hoveredDeleteId === note.id && "border-red-500/50"
          )}
        >
          {/* Scanning Light Effect */}
          <div className="absolute inset-0 bg-linear-to-tr from-cyan-500/5 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700" />

          <div className="relative">
            <div className="absolute inset-0 bg-cyan-500/20 blur-2xl rounded-full animate-pulse" />
            <FileText className="relative h-20 w-20 text-cyan-400 stroke-[0.8] drop-shadow-[0_0_12px_rgba(34,211,238,0.9)]" />
          </div>
          ...
          <div className="absolute top-4 right-4 z-20 opacity-0 group-hover:opacity-100 transition-all duration-300">
            <DropdownMenu>
              <DropdownMenuTrigger asChild onClick={(e) => e.stopPropagation()}>
                <Button variant="ghost" size="icon" className="h-9 w-9 bg-black/40 backdrop-blur-md text-white/50 hover:text-cyan-400 hover:bg-cyan-500/10 rounded-full border border-white/5 hover:border-cyan-500/30">
                  <MoreVertical className="h-5 w-5" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="bg-[#050505]/95 backdrop-blur-xl border-cyan-500/20">
                <DropdownMenuItem onClick={(e) => { e.stopPropagation(); copyNote(note, folderId || undefined); }} className="text-zinc-400 focus:text-cyan-400 focus:bg-cyan-500/10">
      ...
      {/* Metadata below Card */}
      <div className="text-center w-full mt-2 transition-all duration-500 group-hover:translate-y-1">
        <h3 className="font-bold text-lg text-white/90 tracking-tight truncate px-4 group-hover:text-cyan-400 transition-colors">
          {note.title || 'Untitled Project'}
        </h3>
        <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest mt-1 opacity-60 group-hover:opacity-100 transition-opacity">
          Modified {format(note.createdAt, 'd MMM, yyyy')}
        </p>
      </div>
    </motion.div>
  );
...
const FolderCard = React.memo(({ folder, router, hoveredDeleteId, setHoveredDeleteId, setDeleteRequest }: {
  folder: any,
  router: any,
  hoveredDeleteId: string | null,
  setHoveredDeleteId: any,
  ...
  return (
    <motion.div
      ...
      className={cn(
        "group relative flex flex-col items-center justify-center cursor-pointer transition-all duration-500 opacity-0",
    ...
    >
      ...
      <div className="text-center w-full px-2">
        <h3 className="font-bold text-lg text-zinc-900 dark:text-white tracking-tight mb-0.5 truncate drop-shadow-[0_0_5px_rgba(255,255,255,0.2)]">{folder.name}</h3>
        <p className="text-[10px] text-zinc-500 dark:text-cyan-400/50 font-black uppercase tracking-[0.2em]">{folder.notesCount || 0} Files</p>
      </div>
    </motion.div>
  );
});
                            ...
                            <>
                              {isCreatingFolder && (
                                <motion.div
                                  ...
                                  animate={{ scale: 1, opacity: 1 }}
                                  exit={{ scale: 0.8, opacity: 0 }}
                                  className="group relative flex flex-col items-center justify-center cursor-pointer transition-all duration-300 opacity-0"
                                >
                                  <div className="relative w-full aspect-4/3 mb-4 perspective-[1000px] flex items-center justify-center">
                                    {/* Folder 3D Body */}
                                    <div className="absolute w-[80%] h-[70%] bg-linear-to-br from-cyan-600 to-cyan-800 rounded-xl rounded-tl-none shadow-inner transition-transform duration-500">
